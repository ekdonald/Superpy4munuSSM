...
!print*,Mchi(1:3),m12sq,m23sq,sinsqth13,sinsqth12,sinsqth23

!stop
!
!pass normal and inverse hyerarhy
! (Data taken from 1409.5439 by Maltoni, Schwetz)
!!!! Normal Order
!IF (myparam.GT.0._dp) THEN
!Write(338,*) "NH",m2atmNH,m2solNH,sinsqth13,sinsqth12,sinsqth23
!ENDIF
!!!! Exp. limits Normal Order
!IF ((m2solNH.GE.7.02e-5_dp).AND.(m2solNH.LE.8.09e-5_dp)) THEN
!IF ((m2atmNH.GE.2.317e-3_dp).AND.(m2atmNH.LE.2.607e-3_dp)) THEN
!IF ((sinsqth13.GE.0.0186_dp).AND.(sinsqth13.LE.0.0250_dp)) THEN
!IF ((sinsqth12.GE.0.270_dp).AND.(sinsqth12.LE.0.344_dp)) THEN
!IF ((sinsqth23.GE.0.382_dp).AND.(sinsqth23.LE.0.643_dp)) THEN
!
!Write(291,*) m2atmNH,m2solNH,sinsqth13,sinsqth12,sinsqth23
...
!ENDIF
!!!! Inverted Order
!IF (myparam.LT.0._dp) THEN
!Write(339,*) "IH",m2atmIH,m2solIH,sinsqth13,sinsqth12,sinsqth23
!ENDIF
!!!! Exp. limits Inverted Order
!IF ((m2solIH.GE.7.02e-5_dp).AND.(m2solIH.LE.8.09e-5_dp)) THEN
!IF ((m2atmIH.GE.2.307e-3_dp).AND.(m2atmIH.LE.2.590e-3_dp)) THEN
!IF ((sinsqth13.GE.0.0188_dp).AND.(sinsqth13.LE.0.0251_dp)) THEN
!IF ((sinsqth12.GE.0.270_dp).AND.(sinsqth12.LE.0.344_dp)) THEN
!IF ((sinsqth23.GE.0.389_dp).AND.(sinsqth23.LE.0.644_dp)) THEN
!
!Write(292,*) m2atmIH,m2solIH,sinsqth13,sinsqth12,sinsqth23
...
! call constraints(PROB)


!if(any(PROB /= 0)) then 
! Err%coll_constraints = 1  
! return
!endif
...
!!-------------- HiggsBounds Files & HiggsSignals
 call HiggsBounds
  ...
  WRITE(17,*) ' Neutralino ', MChi(1),MChi(5),MChi(10)
...
!!************************************************************************
 ...
 Implicit None 
 integer :: j 
 character (len=200) :: hbfilepath

 hbfilepath = "/lustre/dkpatcha/RpvBayeS4.0.4/contributions/HiggsBounds-4.3.1/example_data/HBout3G/HB3-"
...
!!!**************************************************************
subroutine scan_output
!Use InputOutput_munuSSM3G
!Use Model_Data_munuSSM3G

implicit none

Real(dp) :: mvR(3),mneuvR(3),mneuBino(3),mneuWino(3),mBino,mWino, Mhiggs
 REAL(dp):: PROBtest(100), hu, hd
Integer :: ip, iEL, iH126, iH,iH125
  character(8)  :: date
  character(10) :: time
  character(5)  :: zone
...
!--------------------------------------------------------------
 ...
 OPEN(unit = 17, file = trim(EXPCON_PATH)//"OutputConstraints_"//trim(rankstr)//".dat",status="unknown") 
...
DO iEL = 1, 8
 ...
 WRITE(17,*) '  ' 
 WRITE(17,*) ' Charged Higgs,  Neutalinos'
 DO  iEL = 2, 8
  WRITE(17,*) '  ',  MHpm(iEL),  MChi(iEL+2)
 ENDDO
 
 WRITE(17,*) '  ' 
 WRITE(17,*) ' Charged Higgs, charginos masses '
 DO  iEL = 1, 5
  WRITE(17,*) iEL, '  ',  MCha(iEL)
 ENDDO
...
!!-----------------------------------------------------------------------------------
 OPEN(unit = 23, file = trim(EXPCON_PATH)//"MhForKapLam_"//trim(rankstr)//".dat",status="unknown") 
...
!!-----------------------------------------------------------------------------------
 OPEN(unit = 20, file = trim(EXPCON_PATH)//"MhForvLYv_"//trim(rankstr)//".dat",status="unknown")
