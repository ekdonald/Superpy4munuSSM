...
# This info file can be used for plot labels.\n''')
    ...
    info.write('''# Priors.\n''')
    for key in Priors.MUNUSSMModelTracker().param:
        info.write(
            '''%s %s %s\n''' %
            (key,
             Priors.MUNUSSMModelTracker().param[key].type,
             Priors.MUNUSSMModelTracker().param[key].arg))

    # Write the likelihoods types and arguments.
    info.write('''# Likelihoods.\n''')
    for key in Likelihood.MUNUSSMConstraintTracker().constraint:
        info.write(
            '''%s %s %s\n''' %
            (key,
             Likelihood.MUNUSSMConstraintTracker().constraint[key].type,
             Likelihood.MUNUSSMConstraintTracker().constraint[key].arg))
