...
class Block(object):
    ...
    def add_entry(self, args):
        ...
        #print('before map(_autotype, args)', args) 
        args = list(map(_autotype, args))
        #print('after map(_autotype, args)', args) 
        ## Re-join consecutive strings into single entries
        i = 0
        while i < len(args)-1: # MODIFIED BY DEK
...
class Particle(object):
    ...
    def add_decay(self, br, nda, ids):
        ...
        #self.decays.sort()
        #print ('self.decays.keys() = ', self.decays.keys()) 
        #self.decays = sorted(self.decays, key=itemgetter('first_key', 'second_key'))
...
def readSLHA(spcstr, ignorenobr=False, ignorenomass=False):
    ...
    """
    print('entering readSLHA --- pyslha') 
    ...
    for line in spcstr.splitlines():
        #print('test --- spcstr.splitlines() = ', spcstr.splitlines()) 
        ...
        if not line.startswith(" "):
            ...
            if line.upper().startswith("BLOCK"):					#MODIFIED BLOCK -> Block
                #print line
                match = re.match(r"BLOCK\s+(\w+)(\s+Q\s*=\s*.+)?", line.upper())	#MODIFIED BLOCK -> Block
                #print('test --- BLOCK line.upper() = ', line.upper()) 
                if not match:
                    continue
            ...
            ## Handle DECAY start lines
                ...
                #print('test --- DECAY line.upper() = ', line.upper()) 
        ...
        ## This non-empty line starts with an indent, hence must be an in-section data line
            ...
            #print('test --- non-empty line starts = ') 
    ...
    #print('test decays = ', decays)
    #print('test blocks.keys() = ', blocks) 
    #print('test blocks.keys() = ', blocks.keys()) 
    #print('test blocks["MASS"] = ', blocks['MASS']) 
    #print('test blocks["MASS"].keys() = ', blocks['MASS'].keys()) 
    #print('test blocks["GAUGE"] = ', blocks['GAUGE'])

    '''
    for pid in blocks["MASS"].keys():
        #if decays.has_key(pid):
        if pid in decays:
            # print('pid, decays.has_key(pid) = ', pid, decays[pid]) 
